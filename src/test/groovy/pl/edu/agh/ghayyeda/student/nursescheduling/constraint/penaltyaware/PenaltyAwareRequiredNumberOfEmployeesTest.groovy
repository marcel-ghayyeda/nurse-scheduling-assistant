...
import pl.edu.agh.ghayyeda.student.nursescheduling.constraint.penaltyaware.PenaltyAwareRequiredNumberOfEmployees
...
class PenaltyAwareRequiredNumberOfEmployeesTest extends Specification {
    ...
    def "Should classify as not feasible when no nurse in some hour of day"() {
        ...
        def constraint = PenaltyAwareRequiredNumberOfEmployees.between(validationStartTime, validationEndTime, 1)
    ...
    def "Should classify as not feasible when nurse has a day off"() {
        ...
        def validationEndTime = LocalDateTime.of(LocalDate.of(2018, NOVEMBER, 1), LocalTime.of(18, 0))
        def constraint = PenaltyAwareRequiredNumberOfEmployees.between(validationStartTime, validationEndTime, 1)
        ...
        then:
        !constraintValidationResult.feasible
        constraintValidationResult.penalty == 2
    }
    ...
    def "Should classify as not feasible when no nurse in some hour of day but babysitters present"() {
        ...
        then:
        !constraintValidationResult.feasible
        constraintValidationResult.penalty == 1.35d
    }
    ...
    def "Should classify as feasible when enough employees during day and night"() {
        ...
        def validationStartTime = LocalDateTime.of(LocalDate.of(2018, NOVEMBER, 1), DAY.startTime)
        def validationEndTime = LocalDateTime.of(LocalDate.of(2018, NOVEMBER, 2), NIGHT.endTime)
        def constraint = PenaltyAwareRequiredNumberOfEmployees.between(validationStartTime, validationEndTime, 4)
    ...
    def "Should classify as not feasible when no employee at some time during day"() {
        ...
        then:
        !constraintValidationResult.feasible
        constraintValidationResult.penalty == 1.9d
    }
